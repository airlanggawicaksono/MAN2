...
class JadwalRepository:
    ...
    async def find_jadwal_by_id(self, jadwal_id: UUID) -> Jadwal | None:
        result = await self.db.execute(select(Jadwal).where(Jadwal.jadwal_id == jadwal_id))
        return result.scalar_one_or_none()

    async def list_jadwal_by_semester(self, semester_id: UUID) -> list[Jadwal]:
        result = await self.db.execute(select(Jadwal).where(Jadwal.semester_id == semester_id))
        return list(result.scalars().all())

    async def list_jadwal_by_kelas(self, kelas_id: UUID) -> list[Jadwal]:
        result = await self.db.execute(select(Jadwal).where(Jadwal.kelas_id == kelas_id))
        return list(result.scalars().all())

    async def list_jadwal_by_guru(self, user_id: UUID) -> list[Jadwal]:
        result = await self.db.execute(select(Jadwal).where(Jadwal.guru_user_id == user_id))
        return list(result.scalars().all())
